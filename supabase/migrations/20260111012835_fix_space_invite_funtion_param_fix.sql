-- Fix accept_space_invitation parameter name to match frontend
-- Frontend calls with p_invitation_id, function expects invitation_id
...
DROP FUNCTION IF EXISTS public.reject_space_invitation(UUID);

CREATE OR REPLACE FUNCTION public.reject_space_invitation(p_invitation_id UUID)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
SELECT '✅ Fixed function parameter names to match frontend (p_invitation_id)' as status;