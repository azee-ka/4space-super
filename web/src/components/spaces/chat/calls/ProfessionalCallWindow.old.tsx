...
export function ProfessionalCallWindow(props: ProfessionalCallWindowProps) {
  ...
  // Window positioning - centered by default
  ...
  useEffect(() => {
    if (!isInitialized && windowRef.current) {
  ...
  if (windowState === 'minimized') {
    return (
      <div className="fixed bottom-6 right-6 z-50 bg-black border border-zinc-800 rounded-lg shadow-2xl p-3 w-80">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
            <span className="text-white text-sm font-medium">{props.roomName || 'Call'}</span>
            <span className="text-gray-500 text-xs">• {props.participants.length} in call</span>
          </div>
      ...
      <div className="w-full h-full bg-black border border-zinc-800 rounded-lg shadow-2xl flex flex-col overflow-hidden">
        ...
        <div onMouseDown={startDrag} className="flex-shrink-0 bg-zinc-900 border-b border-zinc-800 px-3 py-2 flex items-center justify-between cursor-move select-none">
          <div className="flex items-center gap-2">
            ...
            <span className="text-zinc-600 text-xs">• {props.participants.length}</span>
            {isRecording && <div className="flex items-center gap-1 ml-2"><div className="w-2 h-2 bg-red-500 rounded-full animate-pulse" /><span className="text-red-400 text-xs">REC</span></div>}
          </div>
          ...
          <div className="flex items-center gap-1">
            ...
            {/* Docking */}
            <div className="flex items-center gap-0.5 bg-zinc-800 rounded p-0.5 mr-2">
              <button onClick={() => setWindowState('docked-left')} className={`w-6 h-6 rounded flex items-center justify-center ${windowState === 'docked-left' ? 'bg-purple-600' : 'hover:bg-zinc-700'}`} title="Dock Left"><div className="w-2 h-3 border-l-2 border-zinc-400" /></button>
              <button onClick={() => setWindowState('docked-right')} className={`w-6 h-6 rounded flex items-center justify-center ${windowState === 'docked-right' ? 'bg-purple-600' : 'hover:bg-zinc-700'}`} title="Dock Right"><div className="w-2 h-3 border-r-2 border-zinc-400" /></button>
              <button onClick={() => setWindowState('docked-bottom')} className={`w-6 h-6 rounded flex items-center justify-center ${windowState === 'docked-bottom' ? 'bg-purple-600' : 'hover:bg-zinc-700'}`} title="Dock Bottom"><div className="h-2 w-3 border-b-2 border-zinc-400" /></button>
              <button onClick={() => setWindowState('floating')} className={`w-6 h-6 rounded flex items-center justify-center ${windowState === 'floating' ? 'bg-purple-600' : 'hover:bg-zinc-700'}`} title="Float"><FontAwesomeIcon icon={faArrowsAlt} className="text-xs text-zinc-400" /></button>
            </div>

            <button onClick={openInNewWindow} className="w-7 h-7 rounded hover:bg-zinc-800 flex items-center justify-center text-cyan-400 hover:text-cyan-300 transition" title="Open in New Window">
